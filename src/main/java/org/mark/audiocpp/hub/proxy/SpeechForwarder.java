...
/**
 * 通用任务转发器：把 {"request":{...}} 包成 {"model":"model","request":{...}}
...
public class SpeechForwarder {
    ...
    public String forward(int port, JsonObject frontendBody) throws IOException, InterruptedException {
        JsonObject body = new JsonObject();
        body.addProperty("model", "model");
        body.add("request", frontendBody.has("request") ? frontendBody.get("request") : new JsonObject());

        HttpRequest httpRequest = HttpRequest.newBuilder()
                .uri(URI.create("http://127.0.0.1:" + port + "/v1/tasks/run"))
                // 懒加载 + 生成可能很慢，给足超时
